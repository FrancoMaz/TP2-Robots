...
contract Class is Ownable, Credits {
    ...
    function createCourse(uint _id, string memory _name, address _prof, uint _credits, uint[] memory _correlatives, bool _active) private {
        require(msg.sender == courseToOwner[_id] && _id > 0 && keccak256(abi.encodePacked(_name)) != keccak256(abi.encodePacked('')) && _prof != '' && _credits > 0);
        Course memory course = Course(_id, _name, _prof, _credits, _correlatives, _active);
        idToCourse[_id] = storage course;
        courseToOwner[_id] = msg.sender;
        courses.push(storage course);
    }
    ...
    function approveStudentPartial(address _student, uint _courseId) public payable {
        require((msg.sender == idToCourse[_courseId].prof || msg.sender == courseToOwner[_courseId]) && correlativesApproved(_student, _courseId));
    ...
    function approveStudentTotal(address _student, uint _courseId, uint _rate) public payable {
        require((msg.sender == idToCourse[_courseId].prof || msg.sender == courseToOwner[_courseId]) && correlativesApproved(_student, _courseId) && _rate >= minRate && _rate <= maxRate);
